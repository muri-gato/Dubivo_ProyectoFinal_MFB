            ...
            <div class="bg-white p-6 rounded-lg shadow-md sticky top-4">
                ...
                <form method="GET" action="{{ route('actors.index') }}" id="filter-form">
                    ...
                    <div class="min-w-[150px]">
                        ...
                        <select name="favorites" class="w-full border border-gray-300 rounded-lg px-3 py-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition duration-300">
                    <div class="relative">
                        ...
                        @auth
                        @if(Auth::user()->role == 'client')
                        <form action="{{ route('actors.favorite.toggle', $actor) }}" method="POST" class="absolute top-2 right-2 z-10">
                            @csrf
                            <button type="submit" class="bg-white bg-opacity-80 rounded-full p-2 hover:bg-opacity-100 transition shadow-sm hover:shadow-md"
                                title="{{ $actor->isFavoritedBy(Auth::id()) ? 'Quitar de favoritos' : 'Añadir a favoritos' }}">
                                <svg class="w-5 h-5 {{ $actor->isFavoritedBy(Auth::id()) ? 'text-red-500 fill-current' : 'text-gray-400 hover:text-red-400' }}"
