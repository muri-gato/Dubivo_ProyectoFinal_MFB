...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="text-center mb-12">
        <h1 class="text-4xl font-bold text-gray-800 mb-4">Banco de Voces</h1>
    ...
    <div class="mb-20">
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="{{ route('actors.index') }}" class="text-azul-profundo hover:text-azul-profundo hover:bg-opacity-90 font-semibold transition duration-200">
            ...
            <div class="relative h-full">
                ...
                <div class="{{ $classes }}" data-index="{{ $index }}">
                    ...
                    <a href="{{ route('actors.show', $actor) }}" 
                       class="block bg-white shadow-md overflow-hidden h-[600px] w-80 mx-auto transform transition-all duration-300 hover:shadow-xl cursor-pointer group border border-gray-200 hover:border-naranja-vibrante hover:scale-105">
                        ...
                        <div class="relative h-[500px] overflow-hidden">
                            ...
                            @else
                                <div class="w-full h-full bg-gray-100 flex items-center justify-center group-hover:bg-gray-200 transition duration-300">
                                    <i class="fas fa-user text-gray-400 text-7xl"></i>
                                </div>
                            @endif
                            ...
                            <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-8">
                                ...
                                <div class="flex space-x-4 text-white text-base">
                                    <span class="flex items-center bg-white/20 backdrop-blur-sm px-4 py-2">
                        ...
                        <div class="p-6 h-[100px] flex items-center justify-between bg-white">
                            <!-- Estadísticas -->
                            <div class="flex space-x-6">
                                <div class="text-center">
                                    <div class="text-2xl font-bold text-naranja-vibrante">{{ $actor->works->count() }}</div>
                                    <div class="text-xs text-gray-600 font-medium mt-1">PROY.</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-2xl font-bold text-ambar">{{ $actor->schools->count() }}</div>
                                    <div class="text-xs text-gray-600 font-medium mt-1">ESC.</div>
                                </div>
                            </div>

                            <!-- Indicador de click -->
                            <div class="text-azul-profundo group-hover:text-naranja-vibrante transition-colors duration-300">
        ...
        <div class="flex justify-center mt-16 space-x-3">
            @foreach($featuredActors as $index => $actor)
            <button class="w-3 h-3 bg-gray-300 hover:bg-azul-profundo transition duration-200 carousel-indicator {{ $index === 0 ? 'bg-azul-profundo' : '' }}" data-index="{{ $index }}"></button>
            @endforeach
        </div>
    </div>
    @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div class="bg-white shadow-md p-6 text-center hover:shadow-lg transition duration-300 border border-gray-200 hover:border-rosa-electrico">
            ...
            <a href="{{ route('actors.index') }}" class="bg-rosa-electrico hover:bg-rosa-electrico hover:bg-opacity-90 text-white px-6 py-2 font-semibold transition duration-200 inline-block">
                Ver Actores
            </a>
        </div>
        ...
        <div class="bg-white shadow-md p-6 text-center hover:shadow-lg transition duration-300 border border-gray-200 hover:border-ambar">
            ...
            <a href="{{ route('schools.index') }}" class="bg-ambar hover:bg-ambar hover:bg-opacity-90 text-white px-6 py-2 font-semibold transition duration-200 inline-block">
                Ver Escuelas
            </a>
        </div>
        ...
        <div class="bg-white shadow-md p-6 text-center hover:shadow-lg transition duration-300 border border-gray-200 hover:border-azul-profundo">
            ...
            <a href="{{ route('works.index') }}" class="bg-azul-profundo hover:bg-azul-profundo hover:bg-opacity-90 text-white px-6 py-2 font-semibold transition duration-200 inline-block">
                Ver Obras
            </a>
        </div>
    </div>
    ...
    <div class="bg-gray-50 border border-gray-200 p-8 text-center">
        ...
        <div class="flex justify-center space-x-4">
            <a href="{{ route('register') }}" class="bg-rosa-electrico hover:bg-rosa-electrico hover:bg-opacity-90 text-white px-8 py-3 font-semibold transition duration-200">
                Registrarse
            </a>
            <a href="{{ route('login') }}" class="bg-white text-azul-profundo border border-azul-profundo px-8 py-3 hover:bg-azul-profundo hover:bg-opacity-10 font-semibold transition duration-200">
                Iniciar Sesión
            </a>
        </div>
    </div>
