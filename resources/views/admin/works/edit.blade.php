        ...
        <form action="{{ route('admin.works.update', $work) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <select name="type" 
                    ...
                    <option value="movie" {{ old('type', $work->type) == 'movie' ? 'selected' : '' }}>Película</option>
                    <option value="series" {{ old('type', $work->type) == 'series' ? 'selected' : '' }}>Serie</option>
                    <option value="commercial" {{ old('type', $work->type) == 'commercial' ? 'selected' : '' }}>Comercial</option>
                    <option value="animation" {{ old('type', $work->type) == 'animation' ? 'selected' : '' }}>Animación</option>
                    <option value="videogame" {{ old('type', $work->type) == 'videogame' ? 'selected' : '' }}>Videojuego</option>
                </select>
                @error('type')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
